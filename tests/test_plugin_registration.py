...
def test_load_and_register_plugins(settings):
    ...
    assert actions["respond_to"][respond_to_key]["regex"] == re.compile(
        "hello", re.IGNORECASE
    )
    ...
    assert actions["listen_to"][listen_to_key]["regex"] == re.compile(
        "hi", re.IGNORECASE
    )
...
def test_plugin_storage_fq_plugin_name(settings):
    machine = Machine(settings=settings)
    actions = machine._plugin_actions
    plugin1_cls = actions["respond_to"][
        "tests.fake_plugins:FakePlugin.respond_function-hello"
    ]["class"]
