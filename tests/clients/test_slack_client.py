...
@pytest.fixture
def user(user_dict):
    return User.from_api_response(user_dict)
...
def channel(channel_dict):
    return Channel.from_api_response(channel_dict)
...
async def test_process_users_channels_team_join(slack_client, socket_mode_client, user_dict):
    ...
    assert slack_client._users["U1"] == User.from_api_response(user_dict)
...
async def test_process_users_channels_channel_created(slack_client, socket_mode_client, web_client, channel_dict):
    ...
    assert slack_client._channels["C1"] == Channel.from_api_response(channel_dict)
