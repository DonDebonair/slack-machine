...
async def test_get(redis_storage, redis_client):
    redis_client.get.expect("SM:key1").returns(make_awaitable_result(None))
...
async def test_has(redis_storage, redis_client):
    redis_client.exists.expect("SM:key1").returns(make_awaitable_result(None))
...
async def test_delete(redis_storage, redis_client):
    redis_client.delete.expect("SM:key1").returns(make_awaitable_result(None))
