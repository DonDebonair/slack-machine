...
@pytest.fixture
def storage(mocker):
    return MagicMock(spec=MachineBaseStorage)
...
def dispatcher(mocker, plugin_actions, request):
    mocker.patch("machine.singletons.Slack", autospec=True)
    mocker.patch("machine.singletons.Scheduler", autospec=True)
...
async def test_handle_event_process(dispatcher, fake_plugin):
    ...
    fake_plugin.process_function.assert_called_once_with({})
