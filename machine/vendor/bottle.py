...
class BaseResponse:
    ...
    expires = HeaderProperty(
        "Expires", reader=lambda x: datetime.utcfromtimestamp(parse_date(x)), writer=lambda x: http_date(x)
    )
        ...
        for key, value in options.items():
            ...
            if key in ("same_site", "samesite"):  # 'samesite' variant added in 0.13
                ...
                if value.lower() not in ("lax", "strict"):
                    raise CookieError(f"Invalid value samesite={key!r} (expected 'lax' or 'strict')")
